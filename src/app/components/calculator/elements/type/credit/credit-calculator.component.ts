...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ListBanksComponent, ReactiveFormsModule],
  templateUrl: "./credit-calculator.component.html",
})
export class CreditCalculatorComponent implements OnInit {
  private windowTitle = "Calculadora de Crédito | Aplicación | Beacon";
  ...
  openBankListModal() {
    ...
    modalRef.componentInstance.calculatorType = 'credito';
    modalRef.componentInstance.bankSelected.subscribe((data: { bank: IBanks | null, customInterest: number | null, calculatorType: string }) => {
      if (data.customInterest !== null) {
        ...
        this.updateBankDetails(data.bank.nombre, data.bank.tasaCredito);
      }
    });
  }
  ...
  calculate() {
    if (this.form.valid) {
      const amount = parseFloat(
        this.form.get("amount")?.value.replace(/,/g, "")
      );
  ...
  onAmountInput(event: Event) {
    ...
    const formattedValue = this.thousandSeparatorService.formatNumber(
      inputElement.value
    );
